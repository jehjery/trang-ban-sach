 ...
 <section class="header">
		...
		<div>
			<ul class="navbar">
				...
				<li><a class="active" href="LOAIcontroller">Type</a></li>
    ...
    <table width="100%">
      <thead>
      	<tr><div class="add-product">
				<a href="ThemLoai.jsp">+ ADĐ More</a>
			</div></tr>
        <tr>
         <td>Chỉnh sửa</td>
           <td>Xóa</td>
          <td>Mã Loai</td>
          <td>tên Loai</td>
          <td>Số Lượng sp còn</td>

        </tr>
      </thead>
      <tbody>
 <% if(request.getAttribute("Loai")!=null){
 		ArrayList<loaibean> listhd = (ArrayList<loaibean>) request.getAttribute("Loai");
	for(loaibean aa: listhd){	
	%>  
        <tr>
           <td><a href="Sualoai.jsp?maloai=<%=aa.getMaloai()%>&tenloai=<%=aa.getTenloai()%>"><i class="far fa-times-circle"></i></a></td>
          <td><a href="XoaLoai?XLun=<%=aa.getMaloai()%>"><i class="far fa-times-circle"></i></a></td>
          <td><%=aa.getMaloai()%></td>
          <td><%=aa.getTenloai()%></td>
           <td><%=aa.getsoluongsanpham()%></td>  
        </tr>
       <%} %>
        <%} %>
      </tbody>
