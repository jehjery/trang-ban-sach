 	...
 	<section class="header">
		...
		<div>
			<ul class="navbar">
				...
				<li class="lg-bag"><a href="dathang"><i
		...
		<% 
			if (request.getAttribute("spPRODUCT")!=null){
			sanphambean sp = (sanphambean)request.getAttribute("spPRODUCT");
			 %>
   <section class="prodetails section-p1">
    <div class="single-pro-image">
      <img src="<%=sp.getAnh() %>" class="MainImg" width = "100%" alt="">
      <div class="small-img-group">
     <%
     if (request.getAttribute("list")!=null){
     	ArrayList<sanphambean> listsp = (ArrayList<sanphambean>)request.getAttribute("list"); 
     	int j;
     	if(listsp.size()>=4){
     	for(j=1;j<=4;j++){
     		sanphambean a= listsp.get(j);
     		%> 
        <div class="small-img-col">
          <img src="<%=a.getAnh() %>" width="100%" class="small-img" alt="">
        </div>
      	<%} %>
      	<%} %>
      	<%} %>
    ...
    <div class="single-pro-details">
      <h6><%=sp.getMaloai()%> / <%=sp.getMasp() %></h6>
      <h4><%=sp.getTensp() %></h4>
      <h2><%=sp.getGia() %> ~ <%=sp.getGia()/20000 %>$ </h2>
      <select>
     ...
     <form action="dathang" method="post" >
	  <input name="soluong"  
				method="post" style = "max: 500px;" type="number" value="1"
				min="1" >
				</input>	
      <button value=<%=sp.getMasp()%> name="masp" class="normal">Add To Cart</button>
	</form>
      <h4>Product Details</h4>
      <span>The Gildan Ultra Cottin T-shirt is made from a subtantial 6.0 oz. per sq. yd. fabric constructed from 100% cotton, tgus ckassic fit preshrunk jersey knit provides unmatched comfort with each wear. featuring a taped neck and shoulder, and a seamless double-needle collar, and available in a range of colors, it offers it all in the ultimate head-turning package.</span>
    </div>
  </section>
<%} %>
  ...
  <section class="product1 section-p1">
    ...
    <div class="pro-container">
		...
		<a class="sanpham" href="PRODUCTcontroller?maloai=<%=l.getMaloai()%>&masp=<%=l.getMasp()%>">
		<div class="pro">
        <img src="<%=l.getAnh() %>" alt="">
        <div class="des">
          <span><%=l.getNguongoc() %></span>
          <h5><%=l.getTensp() %></h5>
          <div class="star">
          ...
          <h4><%=l.getGia()%> VND ( ≈ <%=l.getGia()/20000%>$ )</h4>
        </div>
        <a href="dathang?masp=<%=l.getMasp()%>"><i class="fal fa-shopping-cart cart"></i></a>
      </div>
		<%}%>
		</a>
			<%}%>
    ...
    <p>your susgest</p>
 	 ...
 	 <%
		...
		response.setCharacterEncoding("utf-8");
 		 ArrayList<likerbean> listblog = (ArrayList<likerbean>)request.getAttribute("liker");
	  for(likerbean l: listblog){ %>	
