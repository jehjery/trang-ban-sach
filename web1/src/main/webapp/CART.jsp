  ...
  <section class="header">
		...
		<div>
			<ul class="navbar">
				...
				<li><a href="BUILDcontroller">Build</a></li>
				<li class="active"  class="lg-bag"><a href="dathang"><i
				...
				<li class="use-Avatar"><i class="fa fa-user-circle Avatar"
					...
					{	
						...
						<li><a href="profile.jsp?tendangnhap=<%=a.getTendangnhap()%>&matkhau=<%=a.getMatkhau()%>&fullname=<%=a.getFullname()%>&email=<%=a.getemail()%>&gioitinh=<%=a.getGioitinh()%>&ngay=<%=a.getNgaysinh()%>&thang=<%=a.getThangsinh()%>&nam=<%=a.getNamsinh()%>">
						<%=a.getTendangnhap()%>: <%=a.getFullname() %></a></li>
 ...
 <section class="cart section-p1">
    <table width="100%">
      ...
      <tbody>
        ...
        <tr>
          <td><a href="removeCart?msrm=<%=o.getMasp()%>&rm=1"><i class="far fa-times-circle"></i></a></td>
          <td><img src="<%=o.getAnh() %>" alt=""></td>
          <td>|<%=o.getTensp() %></td>
          <td><%=o.getGia() %></td>
          <td>	 	
		        <form action="UpdateCart" method="post" >
				<input name="number"  
				method="post" style = "max: 500px;" type="number" 
				min="1" value="<%=o.getSoluong()%>">
				</input>
				<button value=<%=o.getMasp()%> name="masach"type="submit"></button>
				</form>
		</td>
          <td><%=o.getThanhtien()%>vnd ≃ <%=o.getThanhtien()/20000 %>$ </td>
        </tr>
       <%} %>
        <%} %>
      </tbody>
  ...
  <section class="cart-add section-p1">
    <div class="coupon">
      ...
      <div>
       <form action="Applyvoucher">
        <input name="voucher" type="text" placeholder="Enter Your Coupon">
    ...
    <div class="subtotal">
      ...
      <table>
        <tr>
          <td>Cart Subtotal</td>
          <td><%=gh.TongTien() %>VND ≃ <%=gh.TongTien()/20000 %>$ </td>
        </tr>
        <tr>
          <td>Shipping</td>
          <td>Free</td>
          ...
          <tr>
          <td>Payment</td>
          <td><select name="payment" class="category-config">
				<label  class="category-product">Payment</label>
				<option value="on delivery" class="category-product" style="text-align: center;" label="Payment on delivery"></option>	
				<option value="on banking"  class="category-product"  style="text-align: center;" label="Banking online"></option>
			</select>
		  </td>
        </tr>
          <tr>
        ...
        %>
          <td><%=gtri*100%>% (<%=voucher %>)</td>
        <%} %>	
        </tr>
        <tr>
          <td><strong>Total</strong></td>
          <td><strong><%=gh.TongTien()*(1-gtri) %>VND ≃ <%=gh.TongTien()/20000 %>$</strong></td>
        </tr>
      </table>
      <div class="a1">
      <form action="Thanhtoan">
      <input type="hidden" name="payment" value="banking"></input>
      <button name="voucher" value="<%=request.getAttribute("voucher")%>" class="normal">Proceed to checkout</button>
      </form>
