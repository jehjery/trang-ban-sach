 ...
 <section class="header">
		...
		<div>
			<ul class="navbar">
				...
				<li><a class="active" href="ADMINcontroller">User</a></li>
...
<section class="cart section-p1">
 ...
 <% if(request.getAttribute("acc")!=null){
 		ArrayList<ACCbean> listhd = (ArrayList<ACCbean>) request.getAttribute("acc");
	for(ACCbean aa: listhd){	
	%>  
        <tr>
          <td><a href="XoaACC?un=<%=aa.getTendangnhap()%>"><i class="far fa-times-circle"></i></a></td>
          <td><%=aa.getTendangnhap()%>/<%=aa.getMatkhau()%></td>
          <td><%=aa.getFullname()%></td>
          <td><%=aa.getNgaysinh()%>/<%=aa.getThangsinh()%>/<%=aa.getNamsinh()%></td>
		  <td><%=aa.getemail()%></td>
          <td><%=aa.getGioitinh()%> </td>
        </tr>
       <%} %>
        <%} %>
      </tbody>
