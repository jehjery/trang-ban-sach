...
//	}
	public ArrayList<sanphambean> Findkey(String key) {
		...
		Bson filter2 = Filters.eq("maloai", key);
		collection.find(filter2).forEach(doc -> 
		{
			Gson gson = new Gson();
			sanphambean sach = gson.fromJson(doc.toJson(), sanphambean.class);
			list.add(sach);
		...
		public Document delete(String masp) {
			Bson filters = Filters.eq("masp", masp);		
...
//		}
		public Document update(sanphambean l) {
			...
			Bson filters = Filters.eq("masp", l.getMasp());		
//			Gson gson = new Gson();
//			Document doc = Document.parse(gson.toJson(l));
