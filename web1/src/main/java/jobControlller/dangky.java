...
public class dangky extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (ACCbean a: listuse)
			{		
				if( (un.equals(a.getTendangnhap() )))
					{
					d++;
					}
			}
		if (	(d==0)
				&&(Integer.parseInt(nam)>=0)
				&&(Integer.parseInt(thang)<=12)
				&&(Integer.parseInt(thang)>=0)
				&&(Integer.parseInt(ngay)<=31)
				&&(Integer.parseInt(ngay)>=0)	)
				{
				 ACC.insert(new ACCbean(un,pass,fullname,Integer.parseInt(nam),Integer.parseInt(thang),Integer.parseInt(ngay),email,gioitinh));
